...
const oraColumns = `JOB_ID, NAME, TRIGGER_TYPE, TRIGGER_VALUE,
...
func (Oracle) UpsertJobSQL(table string) string {
	return fmt.Sprintf(`
		...
		USING (SELECT :1 AS JOB_ID, :2 AS NAME, :3 AS TRIGGER_TYPE, :4 AS TRIGGER_VALUE,
		              :5 AS NEXT_FIRE_TIME, :6 AS STATE, :7 AS ENABLED,
		              :8 AS CREATED_AT, :9 AS UPDATED_AT FROM DUAL) SRC
